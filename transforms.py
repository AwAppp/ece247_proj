...
from scipy.interpolate import interp1d # mycode
...
# Convert EMG Data to PyTorch Tensor
@dataclass
class ToTensor:
...
# Wraps a lambda function inside a transformation
@dataclass
class Lambda:
...
# Apply a Transformation to Each Item in a Batch
@dataclass
class ForEach:
...
# Chains multiple transformations together in sequence
@dataclass
class Compose:
...
# Augment Data by Shifting Electrodes
@dataclass
...
# Introduce Misalignment in EMG Signals
@dataclass
...
# Convert EMG Signals to Frequency Domain
@dataclass
...
class SpatialWarping:
    """Applies spatial warping to simulate electrode misalignment / non-uniform muscle movements
    
    Args:
        shift_range: Max relative shift (as a fraction of electrode count). 
        warp_strength: Strength of non-linear spatial distortion
        channel_dim: Dimension along which the EMG channels are stored
    """
    shift_range: float = 0.2 # can also try 0.1
    warp_strength: float = 0.1 # can also try 0.05
    channel_dim: int = -1

    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        """Applies spatial warping to EMG signals
        
        Args:
            tensor (torch.Tensor): EMG tensor; shape (T, Bands, Channels)
            
        Returns:
            torch.Tensor: Warped EMG tensor w/ spatial distortions
        """
        num_bands, num_channels = tensor.shape[1], tensor.shape[2]

        # generate random shift vals for each channel
        shifts = np.random.uniform(-self.shift_range, self.shift_range, size=(num_bands, num_channels))

        # apply warping using interpolation
        warped_tensor = torch.zeros_like(tensor)  # create a new tensor for warped data
        for b in range(num_bands):  
            for c in range(num_channels):  
                channel_data_np = tensor[:, b, c].cpu().numpy()  # extract (T,) for each band/channel
                T = len(channel_data_np)  # original time length
                x = np.linspace(0, 1, T)  # normalized time axis
                
                # nonlinear warping function
                warped_x = np.clip(
                    x + self.warp_strength * np.sin(2 * np.pi * x + shifts[b, c]),
                    x.min(),
                    x.max()
                )
                warped_x = np.interp(np.linspace(0, 1, len(x)), x, warped_x)  # ensure same length as x

                #print(f"x shape: {x.shape}, warped_x shape: {warped_x.shape}, channel_data_np shape: {channel_data_np.shape}")

                # interpolate; maps original time axis x to channel data
                interp_func = interp1d(x, channel_data_np, kind="linear", fill_value="extrapolate", assume_sorted=False)
                warped_channel_data = interp_func(warped_x)

                # convert back to tensor & assign
                warped_tensor[:, b, c] = torch.tensor(warped_channel_data, dtype=tensor.dtype)
